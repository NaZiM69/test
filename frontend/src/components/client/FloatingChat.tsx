  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi! I'm Doku, your fitness coach. Do you need fitness advice? I'm here to help!",
    },
  ]);
    ...
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end">
      ...
      {isOpen && (
        <div className="mb-4 w-[350px] md:w-[400px] h-[500px] bg-[#0a0a0a] border border-white/10 rounded-[2rem] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
          <div className="p-4 bg-orange-500 flex items-center justify-between text-white">
          ...
          <div className="p-4 border-t border-white/5">
            <form onSubmit={handleSend} className="relative">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-4 pr-12 text-xs focus:outline-none focus:border-orange-500/50 transition-all"
                disabled={isLoading}
              />
              <button
                type="submit"
                disabled={isLoading || !input.trim()}
                className="absolute right-1.5 top-1.5 bottom-1.5 px-3 bg-orange-500 hover:bg-white hover:text-black disabled:opacity-50 rounded-lg text-white transition-all flex items-center justify-center"
              >
                <Send size={14} />
              </button>
            </form>
      ...
      <button
        ...
        className={`w-16 h-16 rounded-full flex items-center justify-center shadow-2xl transition-all hover:scale-110 active:scale-95 ${
          isOpen
            ? "bg-white text-black"
            : "bg-orange-500 text-white shadow-orange-500/20 shadow-xl"
        }`}
      >
