...
import { Bell, AlertCircle, Clock, CheckCircle2, MessageSquare, X } from 'lucide-react';
...
const NotificationCenter: React.FC = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const filteredData = problemsRes.data.filter((p: any) => !clearedIds.includes(p.id));

      const formatted: NotificationItem[] = filteredData.slice(0, 10).map((p: any) => {
        ...
        return {
          ...
          message: p.problem.length > 60 ? p.problem.substring(0, 60) + '...' : p.problem,
          date: p.date,
          type: 'problem'
        };
      });
  ...
  const handleClearAll = () => {
    ...
    const existingCleared = JSON.parse(localStorage.getItem('cleared_notifications') || '[]');
    localStorage.setItem('cleared_notifications', JSON.stringify([...existingCleared, ...currentIds]));
    setNotifications([]);
    setUnreadCount(0);
  };
  ...
  const handleClearOne = (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
    const existingCleared = JSON.parse(localStorage.getItem('cleared_notifications') || '[]');
    localStorage.setItem('cleared_notifications', JSON.stringify([...existingCleared, id]));
          ...
          <div className="p-5 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            <div className="flex flex-col gap-0.5">
              <h3 className="font-bold text-white flex items-center gap-2">
                Notifications
                <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 text-[10px] font-black uppercase">
                  Live
                </span>
              </h3>
              ...
              notifications.map((n) => (
                ...
                >
                  ...
                  <div className="flex gap-4">
                    <div className="mt-1 w-10 h-10 rounded-xl bg-blue-500/10 text-blue-500 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                      <AlertCircle size={20} />
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm font-bold text-white group-hover:text-blue-400 transition-colors">{n.title}</p>
