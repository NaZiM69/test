...
export default function BicepCoach({ onBack }: CoachProps) {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError("Failed to analyze video. Make sure the backend bicep_service is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto w-full p-8 bg-[#111111] border border-white/10 rounded-3xl shadow-2xl relative overflow-hidden mt-6">
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl font-extrabold bg-gradient-to-r from-brand-light via-brand to-brand bg-clip-text text-transparent mb-4 tracking-tight">AI Bicep Coach</h1>
        <p className="text-neutral-400 font-medium">Upload a video of your bicep curls for real-time form analysis</p>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-brand/10 border border-brand/20 rounded-xl flex items-center gap-3">
                ...
                <p className="text-sm text-neutral-300">
                  We've marked your form in the video. <span className="text-brand font-bold">Green</span> means correct movement, <span className="text-red-500 font-bold">Red</span> means your form needs improvement.
                </p>
              </div>
            </div>
          )}
