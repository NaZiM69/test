...
import { Plus, Trash2, Edit2, Filter, Download, UserPlus, Search, CheckCircle2, XCircle, Phone, Calendar, Mail, Shield, AlertTriangle, CreditCard } from 'lucide-react';
...
const ClientsPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Failed to create client. Please check if username/email already exists.');
    }
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Manage your members and their active subscriptions.</p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold transition-all shadow-lg shadow-blue-500/20 active:scale-95"
        >
          <UserPlus size={18} />
          Add New Client
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'Total Members', value: users.length, color: 'text-blue-500', icon: <UserPlus size={20} /> },
          { label: 'Active Subscriptions', value: users.filter(u => u.role === 'CLIENT' && u.plan_name !== 'No Plan').length, color: 'text-green-500', icon: <CheckCircle2 size={20} /> },
          { label: 'Staff Members', value: users.filter(u => u.role === 'ADMIN').length, color: 'text-purple-500', icon: <Shield size={20} /> },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500 italic">Loading data...</td>
                </tr>
              ) : users.length === 0 ? (
                <tr>
                ...
                <tr key={user.id} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-blue-600/10 text-blue-500 flex items-center justify-center font-bold text-sm border border-blue-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-500 font-mono">ID: #USR-{user.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                       ...
                       <span className={`text-sm ${user.plan_name === 'No Plan' ? 'text-gray-600 italic' : 'text-gray-300 font-medium'}`}>
                        {user.plan_name}
                       </span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                       <button 
                        ...
                        className="p-2 bg-white/5 rounded-lg text-gray-400 hover:text-white hover:bg-blue-600/20 transition-all"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-[#0f0f0f] border border-white/10 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
              <h2 className="text-xl font-bold">{isEditModalOpen ? 'Edit Client Profile' : 'Register New Client'}</h2>
              <button 
            ...
            <form onSubmit={isEditModalOpen ? handleUpdate : handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-blue-500 uppercase tracking-widest mb-2">Account Info</h3>
                <div>
              ...
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-green-500 uppercase tracking-widest mb-2">Personal Info</h3>
              ...
              <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-white/5">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[#151515] border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500 appearance-none"
                  >
                ...
                {!isEditModalOpen && (
                  <>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Select Plan</label>
                      <select 
              ...
              <div className="md:col-span-2 flex gap-3 pt-4">
                ...
                <button 
                  type="submit"
                  className="flex-1 px-6 py-4 rounded-xl bg-blue-600 font-bold hover:bg-blue-500 transition-all shadow-lg shadow-blue-500/20"
                >
          ...
          <div className="bg-[#0f0f0f] border border-red-500/20 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl shadow-red-500/10">
            <div className="p-8 text-center">
              ...
              <p className="text-gray-400 text-sm mb-8 leading-relaxed">
                Are you sure you want to delete <span className="text-white font-bold">{userToDelete.username}</span>? This action is permanent and will remove all associated records.
              </p>
